		...
		if ( pSocket == pThis->pSocket )
		{
			...
			if ( pNewSocket )
			{
				if ( pThis->handle_event )
				{
					...
					iRetCode = pThis->handle_event( pThis, EVENT_ACCEPT_NEW_STREAM_CLIENT_NOTIFY, &stEventParam, sizeof(stEventParam), pThis->pUserData );
				}
			}
			else 
			{
				if ( SOCKET_ERROR == iRetCode )
				{
					if ( pThis->handle_event )
					{
						pThis->handle_event( pThis, EVENT_ACCEPT_ERROR, NULL, 0, pThis->pUserData );
					}
				}
			}
		}
