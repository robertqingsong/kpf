...
int32_t iRetCode = -1;
	...
	if ( iUDPReactorId )
	{
		...
		{
			CSocketParam stSocketParam;
			
			memset( &stSocketParam, 0x00, sizeof(stSocketParam) );
			memcpy( stSocketParam.pIP, pLocalIP, strlen(pLocalIP) );
			stSocketParam.iPort = LOCAL_PORT;
			
			if ( net_set_socket( iUDPServiceSocketId, SOCKET_OPTION_SET_MULTICAST, &stSocketParam, sizeof(stSocketParam) ) < 0 )
			{
				...
				log_print( "set multicast address ok..............." );
			...
			log_print( "multicast listener is running.............." );
		}
