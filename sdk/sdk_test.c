...
int32_t reactor_callback( int32u_t iReactorId, int32_t iSocketId, void *pUserData )
{
	int32_t iRetCode = -1;

	printf( "reactor_callback:##################################>\r\n" );
	
	log_print( "pUserData->%d.\r\n", (int)pUserData );
...
int main( int argc, char **argv )
{
	int32u_t iSocketId = net_socket( SOCKET_TYPE_STREAM, 0 );
	CNetAddr addr;
	...
	if ( iSocketId > 0 )
	{
		int32u_t iReactorId = 0;
		const int8_t *pDomainName = "www.baidu.com";
		const int8_t *pData = NULL;
		int32_t iOpRet = -1;
		
		
		memcpy( addr.pIP, pDomainName, strlen(pDomainName) );
		addr.iPort = 80;
		

		if ( net_connect( iSocketId, &addr ) >= 0 )
			printf( "connect to server ok..............\r\n" );
		else 
			printf( "connect to server failed?????????????????????????\r\n" );
			
		if ( net_set_socket( iSocketId, SOCKET_OPTION_NONE_BLOCK, NULL, 0 ) >= 0 )
			log_print( "set socket none block ok................" );
		else
			log_print( "set socket none block failed???????????????????" );
		
		iReactorId = net_reactor(  );
		if ( iReactorId )
			log_print( "create reactor ok................" );
		else 
		{
			log_print( "create reactor failed????????????????" );
			
			return -1;	
		}
		
		register_reactor_callback( iReactorId, reactor_callback, 199 );

		add_reactor_socket( iReactorId, iSocketId, NULL );
		
		pData = "teststringdsaldjslf\r\n\r\n";
		iOpRet = net_send( iSocketId, pData, strlen(pData) );
		if ( iOpRet >= 0 )
			printf( "send data ok.%d bytes................\r\n", iOpRet );
		else 
			printf( "send data failed???????????????????????\r\n" );

		remove_reactor_socket( iReactorId, iSocketId );
		
		net_close_reactor( iReactorId );
	}
