...
int32_t dgram_session_business( const struct CSession_t *pThis, 
				    const int8u_t *pInData, 
				    const int32_t iInDataLen )
{
	int32_t iRetCode = -1;

	log_print( "dgram_session_business:------------------------>" );	
	...
	log_print( "dgram_session_business<------------------------end" );	
	
	return iRetCode;	
}

int32_t dgram_session_event( const struct CSession_t *pThis, 
				 ...
				 const int32_t iEventParamSize )
{
	...
	log_print( "dgram_session_event:------------------------>" );
	
	log_print( "dgram_session_event<------------------------end" );	
...
int main( int argc, char **argv )
{
	...
	pUDPSession = create_session( SESSION_TYPE_DGRAM_CLIENT, &stUDPSessionParam );
	...
	if ( pUDPSession )
	{
		...
		if ( set_session_business( pUDPSession, dgram_session_business ) >= 0 )
		{
			if ( set_session_event( pUDPSession, dgram_session_event ) >= 0 )
			{
				...
				{
					int32_t iNSent = send_session_data( pUDPSession, pWords, strlen(pWords) + 1 );
